...
function AcervoPage() {
  ...
  // Extrair movimentos únicos para o select (removendo duplicatas e valores nulos)
  const movimentosUnicos = Array.from(new Set(songs.map((s) => s.movimento).filter(Boolean)));

  // Lógica de filtragem
  const filteredSongs =
    filterMovimento === "Todos" ? songs : songs.filter((s) => s.movimento === filterMovimento);
      ...
      <div className="mb-8 flex items-center">
        <div className="relative">
          <select
            ...
            className="appearance-none border-2 border-ink bg-paper py-3 pl-4 pr-12 font-typewriter text-xs uppercase tracking-widest text-ink transition-colors hover:bg-ink hover:text-paper focus:outline-none focus:ring-0 cursor-pointer shadow-[4px_4px_0_0_var(--ink)]"
          >
        ...
        {paginatedSongs.length > 0 ? (
          paginatedSongs.map((song) => (
            <motion.div
              ...
              className="group flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 border-2 border-ink bg-card p-4 transition-transform hover:-translate-y-1 hover:shadow-[4px_4px_0_0_var(--ink)]"
            >
              {/* Capa */}
